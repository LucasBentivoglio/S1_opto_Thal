...
#define nrn_init _nrn_init__kdrin
#define _nrn_initial _nrn_initial__kdrin
#define nrn_cur _nrn_cur__kdrin
#define _nrn_current _nrn_current__kdrin
#define nrn_jacob _nrn_jacob__kdrin
#define nrn_state _nrn_state__kdrin
#define _net_receive _net_receive__kdrin 
#define rate rate__kdrin 
#define states states__kdrin 
...
#define ki _p[8]
#define ki_columnindex 8
#define ko _p[9]
#define ko_columnindex 9
#define v _p[10]
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kdrin", _hoc_setdata,
 "alf_kdrin", _hoc_alf,
 "bet_kdrin", _hoc_bet,
 "rate_kdrin", _hoc_rate,
 0, 0
};
#define alf alf_kdrin
#define bet bet_kdrin
 extern double alf( _threadargsprotocomma_ double );
 extern double bet( _threadargsprotocomma_ double );
 /* declare global and static user variables */
 /* some parameters have upper and lower limits */
 static HocParmLimits _hoc_parm_limits[] = {
 "gkdrbar_kdrin", 0, 1e+09,
 0,0,0
};
 static HocParmUnits _hoc_parm_units[] = {
 "gkdrbar_kdrin", "mho/cm2",
 "ik_kdrin", "mA/cm2",
 "gk_kdrin", "mho/cm2",
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 kdrin /home/lucas-eduardo-bentivoglio/Music/S1_opto_Thal/mod2/kdrin.mod\n");
...
double alf ( _threadargsprotocomma_ double _lv ) {
 ...
 _lva = _lv - 13.0 ;
   if ( fabs ( _lva ) < 1e-04 ) {
     _lva = _lva + 0.0001 ;
     _lalf = ( - 0.018 * _lva ) / ( - 1.0 + exp ( - ( _lva / 25.0 ) ) ) ;
     }
   else {
     _lalf = ( - 0.018 * ( _lv - 13.0 ) ) / ( - 1.0 + exp ( - ( ( _lv - 13.0 ) / 25.0 ) ) ) ;
     }
...
double bet ( _threadargsprotocomma_ double _lv ) {
 ...
 _lvb = _lv - 23.0 ;
   if ( fabs ( _lvb ) < 1e-04 ) {
     _lvb = _lvb + 0.0001 ;
     _lbet = ( 0.0054 * _lvb ) / ( - 1.0 + exp ( _lvb / 12.0 ) ) ;
     }
   else {
     _lbet = ( 0.0054 * ( _lv - 23.0 ) ) / ( - 1.0 + exp ( ( _lv - 23.0 ) / 12.0 ) ) ;
     }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/lucas-eduardo-bentivoglio/Music/S1_opto_Thal/mod2/kdrin.mod";
static const char* nmodl_file_text = 
